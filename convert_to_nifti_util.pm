...
# could use image name (suffix) to figure out datatype
  ...
  if ($xdim eq "NO_KEY" || $zdim eq "NO_KEY" || $ydim eq "NO_KEY" || $xfov_mm eq "NO_KEY")
  {
      error_out("Could not find good value for xyz or xfov\n\tx=$xdim, y=$ydim, z=$zdim, xfov=$xfov_mm\n");
  }
...
# ------------------
{
  ...
  {  
      $args =   "\'$src_image_path\', \'$image_prefix\', \'$image_suffix\', \'$dest_nii_path\', $xdim, $ydim, $zdim, $nii_datatype_code, $voxel_size, $flip_y, $flip_z";
  }
  else
  {
