...
get single value from bruker ehader.
...
displays the whole bruker header all pretty froma bruker_header_ref
print keys
headfile or pretty

=cut
###
sub display_header { # ( $brukerhash_ref,$indent,$format,$pathtowrite ) 
###
    my ($bruker_header_ref,$indent,$format,$file)=@_;
    debugloc();
    my @bfkeys=qw/a b/;
    ...
    if ( $#bfkeys == -1 ) { 
        ...
        foreach my $key (sort keys %{$bruker_header_ref} ) {
            if ( $format eq "headfile" ) {
                ...
                print "${indent}$key =\n";
                $value=aoaref_to_singleline($bruker_header_ref->{$key});
...
###
sub array_find_by_length { # ( $brukerhash_ref,$arraylength)
###
=item array_find_by_length

searches the whole bruker header from a bruker_header_ref for arrays
$arraylength

=cut

    my ($bruker_header_ref,$indent,$arraylength)=@_;
    debugloc();
    my @bfkeys=qw/a b/;
    @bfkeys=keys(%{$bruker_header_ref});
    my $value="test";
    ...
    if ( $#bfkeys == -1 ) { 
        ...
        foreach my $key (sort keys %{$bruker_header_ref} ) {
            my $aoaref=$$bruker_header_ref{$key};
            my $aoalength=$#{$aoaref}+1;
            my $subarraylength=$#{$aoaref->[0]};
#           print("aoaref$aoaref\n");
#           my $temp=$#{${bruker_header_ref}{$key}};
#           ($#{$bruker_header_ref->$key} +1)
...
###
sub single_find_by_value { # ( $brukerhash_ref,$arraylength)
###
=item single_find_by_value

searches the whole bruker header from a bruker_header_ref for value
$value

=cut
    my ($bruker_header_ref,$indent,$testvalue)=@_;
    debugloc();
    my @bfkeys=qw/a b/;
    @bfkeys=keys(%{$bruker_header_ref});
    my $value="test";
